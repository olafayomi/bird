...
%token DEFINE ON OFF YES NO S MS US PORT
...
%token TIMEFORMAT ISO OLD SHORT LONG BASE NAME CONFIRM UNDO CHECK TIMEOUT
%token LATENCY LIMIT WATCHDOG
...
%token KERNEL PERSIST SCAN TIME LEARN DEVICE ROUTES GRACEFUL RESTART KRT_SOURCE KRT_METRIC

/* Declarations from ../../sysdep/linux/netlink.Y */

%token TABLE KRT_PREFSRC KRT_REALM
...
%token RECEIVE ACTION WARN BLOCK DISABLE KEEP FILTERED
%token PASSWORD FROM PASSIVE TO EVENTS PACKETS PROTOCOLS INTERFACES
%token PRIMARY STATS COUNT FOR COMMANDS PREEXPORT NOEXPORT GENERATE ROA
%token LISTEN BGP V6ONLY DUAL ADDRESS PASSWORDS DESCRIPTION SORTED
%token RELOAD IN OUT MRTDUMP MESSAGES RESTRICT MEMORY IGP_METRIC CLASS DSCP
%token WAIT MAX FLUSH AS
...
%token FUNCTION PRINT PRINTN UNSET RETURN ACCEPT REJECT QUITBIRD INT BOOL IP PREFIX PAIR QUAD EC SET STRING BGPMASK BGPPATH CLIST ECLIST IF THEN ELSE CASE TRUE FALSE RT RO UNKNOWN GENERIC GW NET MASK PROTO SOURCE SCOPE CAST DEST IFNAME IFINDEX LEN DEFINED CONTAINS RESET PREPEND FIRST LAST MATCH ROA_CHECK EMPTY WHERE
...
%token BFD MIN IDLE RX TX INTERVAL MULTIPLIER MULTIHOP NEIGHBOR DEV LOCAL
...
%token HOLD CONNECT RETRY KEEPALIVE STARTUP VIA NEXT HOP SELF DEFAULT PATH METRIC START DELAY FORGET AFTER BGP_PATH BGP_LOCAL_PREF BGP_MED BGP_ORIGIN BGP_NEXT_HOP BGP_ATOMIC_AGGR BGP_AGGREGATOR BGP_COMMUNITY BGP_EXT_COMMUNITY RR RS CLIENT CLUSTER AS4 ADVERTISE IPV4 CAPABILITIES PREFER OLDER MISSING LLADDR DROP IGNORE REFRESH INTERPRET COMMUNITIES BGP_ORIGINATOR_ID BGP_CLUSTER_LIST IGP GATEWAY RECURSIVE MED TTL SECURITY DETERMINISTIC SECONDARY ALLOW PATHS AWARE LINK
...
%token BUFFER LARGE NORMAL STUBNET TAG EXTERNAL
%token LSADB ECMP WEIGHT NSSA TRANSLATOR STABILITY
%token GLOBAL LSID INSTANCE REAL NETMASK LENGTH
%token MERGE LSA SUPPRESSION
...
%token RIP INFINITY PERIOD GARBAGE MULTICAST QUIET NOLISTEN VERSION1 PLAINTEXT MD5 HONOR NEVER ALWAYS RIP_METRIC RIP_TAG
...
%token MULTIPATH BLACKHOLE UNREACHABLE


/* Declarations from ../../proto/sdn/config.Y */

%token SDN UNIXSOCKET
...
/* Grammar from ../../proto/sdn/config.Y */
...
conf: ';' | definition | log_config | mrtdump_base | timeformat_base | debug_unix | rtrid | listen | gr_opts | newtab | roa_table | proto | debug_default | filter_def | filter_eval | function_def ;
cli_cmd: cmd_CONFIGURE | cmd_CONFIGURE_SOFT | cmd_CONFIGURE_CONFIRM | cmd_CONFIGURE_UNDO | cmd_CONFIGURE_CHECK | cmd_DOWN | cmd_SHOW_STATUS | cmd_SHOW_MEMORY | cmd_SHOW_PROTOCOLS | cmd_SHOW_PROTOCOLS_ALL | cmd_SHOW_INTERFACES | cmd_SHOW_INTERFACES_SUMMARY | cmd_SHOW_ROUTE | cmd_SHOW_ROA | cmd_SHOW_SYMBOLS | cmd_ADD_ROA | cmd_DELETE_ROA | cmd_FLUSH_ROA | cmd_DUMP_RESOURCES | cmd_DUMP_SOCKETS | cmd_DUMP_EVENTS | cmd_DUMP_INTERFACES | cmd_DUMP_NEIGHBORS | cmd_DUMP_ATTRIBUTES | cmd_DUMP_ROUTES | cmd_DUMP_PROTOCOLS | cmd_EVAL | cmd_ECHO | cmd_DISABLE | cmd_ENABLE | cmd_RESTART | cmd_RELOAD | cmd_RELOAD_IN | cmd_RELOAD_OUT | cmd_DEBUG | cmd_MRTDUMP | cmd_RESTRICT | cmd_SHOW_BFD_SESSIONS | cmd_SHOW_OSPF | cmd_SHOW_OSPF_NEIGHBORS | cmd_SHOW_OSPF_INTERFACE | cmd_SHOW_OSPF_TOPOLOGY | cmd_SHOW_OSPF_TOPOLOGY_ALL | cmd_SHOW_OSPF_STATE | cmd_SHOW_OSPF_STATE_ALL | cmd_SHOW_OSPF_LSADB | cmd_SHOW_STATIC ;
proto: kern_proto '}' | kif_proto '}' | dev_proto '}' | bfd_proto | bgp_proto '}' { bgp_check_config(BGP_CFG); }  | ospf_proto '}' { ospf_proto_finish(); }  | pipe_proto '}' | rip_cfg '}' { RIP_CFG->passwords = get_passwords(); }  | static_proto '}' | sdn_cfg '}' ;
kern_proto: kern_proto_start proto_name '{' | kern_proto proto_item ';' | kern_proto kern_item ';' | kern_proto kern_sys_item ';' ;
